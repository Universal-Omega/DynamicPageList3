...
class DPL {
	...
	function __construct( $headings, $bHeadingCount, $iColumns, $iRows,
		...
		$listmode, $bescapelinks, $baddexternallink, $includepage,
		...
		if ( !empty( $headings ) ) {
			if ( $iColumns != 1 || $iRows != 1 ) {
					...
					do {
						...
						if ( $greml > 0 ) {
							$this->mOutput .= $this->formatList(
								...
								$defaultTemplateSuffix, $bIncludeTrim,
							...
							$this->mOutput .= $this->formatList(
								$nstart - $offset, $portion + $greml,
								$iTitleMaxLen, $defaultTemplateSuffix,
								$bIncludeTrim, $iTableSortCol, $updateRules,
								$deleteRules
							);
				...
				foreach ( $headings as $heading => $headingCount ) {
					...
					$this->mOutput .= $this->formatList(
						$headingStart, $headingCount, $iTitleMaxLen,
						$defaultTemplateSuffix, $bIncludeTrim, $iTableSortCol,
	...
	function substTagParm( $tag, $pagename, $article, $imageUrl, $nr, $titleMaxLength ) {
		...
		if ( $article->mSelTitle != '' ) {
	    	if ( $article->mSelNamespace == 0 ) {
		    	...
		    	$sTag = str_replace(
					'%PAGESEL%',
					$this->nameSpaces[$article->mSelNamespace] . ':' . str_replace( '_', ' ', $article->mSelTitle ),
					$sTag
				);
	    	}
    	}
	...
	function formatList( $iStart, $iCount, $iTitleMaxLen,
		...
		for ( $i = $iStart; $i < $iStart + $iCount; $i++ ) {
			...
			if ( $article->mNamespace == 6 ) {
				...
				$img = wfFindFile( $article->mTitle->getText() );
				if ( $img && $img->exists() ) {
					$imageUrl = $img->getURL();
			...
			if ( $this->mEscapeLinks &&
				( $article->mNamespace == 14 || $article->mNamespace == 6 )
			) {
			...
			if ( $this->mIncPage ) {
				...
				if ( empty( $this->mIncSecLabels ) || $this->mIncSecLabels[0] == '*' ) { // include whole article
					...
					if ( ( count( $this->mIncSecLabelsMatch ) <= 0 || $this->mIncSecLabelsMatch[0] == '' ||
						...
						if ( $updateRules != '' ) {
							$message = $this->updateArticleByRule(
								$title, $text, $updateRules
							);
							// append update message to output
							$incwiki .= $message;
    					} elseif ( $deleteRules != '' ) {
							$message = $this->deleteArticleByRule(
								$title, $text, $deleteRules
							);
	    					...
	    					if ( array_key_exists( '0', $mode->sSectionTags ) ) {
								$incwiki .= $this->substTagParm(
									$mode->sSectionTags[0], $pagename,
					...
					foreach ( $this->mIncSecLabels as $s => $sSecLabel ) {
						...
						if ( $sSecLabel == '-' ) {
							...
							$secPieces = DPLInclude::includeHeading(
								$this->mParser, $article->mTitle->getPrefixedText(),
								substr( $sSecLabel, 1 ), '', $sectionHeading,
								false, $maxlen, $cutLink,
							...
							$secPieces = DPLInclude::includeTemplate(
								...
								$template2, $template2 . $defaultTemplateSuffix,
								$mustMatch, $mustNotMatch, $this->mIncParsed,
								$iTitleMaxLen, implode( ', ', $article->mCategoryLinks )
							);
							...
							$secPieces = DPLInclude::includeSection(
								$this->mParser, $article->mTitle->getPrefixedText(),
								$sSecLabel, '', false, $bIncludeTrim,
								$skipPattern
							);
 							$secPiece[$s] = implode( isset( $mode->aMultiSecSeparators[$s] ) ?
 								$this->substTagParm( $mode->aMultiSecSeparators[$s], $pagename, $article, $imageUrl, $this->filteredCount, $iTitleMaxLen ) : '',
								$secPieces
							);
 							if ( $mode->iDominantSection >= 0 && $s == $mode->iDominantSection && count( $secPieces ) > 1 ) {
								$dominantPieces = $secPieces;
						...
						if ( count( $mode->sSectionTags ) == 1 ) {
							...
							$septag[$s * 2] = str_replace(
								...
								$sectionHeading[0],
								$this->substTagParm(
									$mode->sSectionTags[$s * 2], $pagename, $article, $imageUrl, $this->filteredCount, $iTitleMaxLen ) );
						} else {
							$septag[$s * 2] = '';
						}
						if ( isset( $mode->sSectionTags[$s * 2 + 1 ] ) ) {
							$septag[$s * 2 + 1] = str_replace(
								...
								$sectionHeading[0],
								$this->substTagParm(
									$mode->sSectionTags[$s * 2 + 1], $pagename,
									$article, $imageUrl, $this->filteredCount,
									$iTitleMaxLen
								)
							);
			...
			if ( $mode->name == 'userformat' ) {
				$rBody .= $this->substTagParm(
					$mode->sItemStart, $pagename, $article, $imageUrl,
					$this->filteredCount, $iTitleMaxLen
				);
			} else {
				...
				if ( $article->mDate != '' ) {
					if ( $article->myDate != '' ) {
						if ( $article->mRevision != '' ) {
							$rBody .= ' <html>' . $sk->makeKnownLinkObj(
								$article->mTitle, htmlspecialchars( $article->myDate ),
								'oldid=' . $article->mRevision ) . '</html> ';
						} else {
						...
						if ( $article->mRevision != '' ) {
							$rBody .= ' <html>' . $sk->makeKnownLinkObj(
								$article->mTitle, htmlspecialchars( $wgLang->timeanddate( $article->mDate, true ) ),
								'oldid=' . $article->mRevision
							) . '</html> : ';
						} else {
			...
			if ( $mode->name == 'userformat' ) {
				$rBody .= $this->substTagParm(
					$mode->sItemEnd, $pagename, $article, $imageUrl,
					$this->filteredCount, $iTitleMaxLen
				);
			} else {
				$rBody .= $mode->sItemEnd;
		...
		if ( $template != '' ) {
			...
			if ( $exec == 'edit' ) {
				...
				if ( $legendPage != '' ) {
					...
					$legendText = preg_replace(
						'/^.*?\<section\s+begin\s*=\s*legend\s*\/\>/s',
						'',
						$legendText
					);
					$legendText = preg_replace(
						'/\<section\s+end\s*=\s*legend\s*\/\>.*/s',
						'',
						$legendText
					);
				}
					...
					foreach ( $parameter as $nr => $parm ) {
						...
						if ( $legendText != '' && $myToolTip == '' ) {
							$myToolTip = preg_replace(
								'/^.*\<section\s+begin\s*=\s*' . preg_quote( $parm, '/' ) . '\s*\/\>/s',
								'',
								$legendText
							);
							if ( strlen( $myToolTip ) == strlen( $legendText ) ) {
								...
								$myToolTip = preg_replace(
									'/\<section\s+end\s*=\s*' . preg_quote( $parm, '/' ) . '\s*\/\>.*/s',
									'',
									$myToolTip
								);
							}
	...
	function updateArticle( $title, $text, $summary ) {
  	...
  	function editTemplateCall( $text, $template, $call, $parameter, $type,
		$value, $format, $legend, $instruction, $optional, $fieldFormat
	) {
		$matches = array();
		...
		$textArea = '<textarea name="' . urlencode( $call . '_' . $parameter ) . "\" $format/>"
			. htmlspecialchars( $value ) . '</textarea>';
		return str_replace(
			'%NAME%', htmlspecialchars( str_replace( '_', ' ', $parameter ) ),
	 ...
	 */
  	function getTemplateParmValues( $text, $template ) {
		$matches = array();
//		$noMatches = preg_match_all('/\{\{\s*'.preg_quote($template,'/').'\s*((\<!--).*?(--\>)\s*)*[|}]/i',$text,$matches,PREG_OFFSET_CAPTURE);
		$noMatches = preg_match_all(
			'/\{\{\s*' . preg_quote( $template, '/' ) . '\s*[|}]/i',
			$text,
			$matches,
			PREG_OFFSET_CAPTURE
		);
	 ...
	 */
  	function updateTemplateCall( &$matchCount, $text, $template, $call,
		$parameter, $value, $afterParm, $optional
	) {
	...
	function articleLink( $tag, $article, $iTitleMaxLen ) {
		...
		return $this->substTagParm(
			$tag, $pagename, $article, $this->filteredCount, '', $iTitleMaxLen
		);
 	}
	 ...
	 *
	 * @param $imgName String: name of the image (may start with Image: or File:)
	 * @return $uniq_prefix
	 */
