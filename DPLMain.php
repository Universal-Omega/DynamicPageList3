...
class DPLMain {
	...
	*/
	...
	public static function dynamicPageList( $input, $params, &$parser, &$bReset, $calledInMode ) {
		...
		// Extension variables
		// Allowed namespaces for DPL: all namespaces except the first 2:
		// Media (-2) and Special (-1), because we cannot use the DB for these
		// to generate dynamic page lists.
		if ( !is_array( ExtDynamicPageList::$allowedNamespaces ) ) { // Initialization
			...
			if ( !is_array( ExtDynamicPageList::$options['namespace'] ) ) {
				...
				ExtDynamicPageList::$options['namespace'] = array_intersect(
			...
			if ( !is_array( ExtDynamicPageList::$options['notnamespace'] ) ) {
				...
				ExtDynamicPageList::$options['notnamespace'] = array_intersect(
					ExtDynamicPageList::$options['notnamespace'], ExtDynamicPageList::$allowedNamespaces
				);
			}
		...
		if ( ExtDynamicPageList::$behavingLikeIntersection ) {
			...
			ExtDynamicPageList::$options['ordermethod'] = array(
				'default' => 'titlewithoutnamespace', 'counter', 'size',
					'category', 'sortkey', 'category,firstedit',
					'category,lastedit', 'category,pagetouched',
					'category,sortkey', 'categoryadd', 'firstedit', 'lastedit',
					'pagetouched', 'pagesel', 'title', 'titlewithoutnamespace',
					'user', 'user,firstedit', 'user,lastedit', 'none'
			);
		...
		foreach ( $aParams as $iParam => $sParam ) {
			...
			if ( $sArg == '' ) {
				if (
					$sType != 'namespace' && $sType != 'notnamespace' &&
					$sType != 'category' &&
					array_key_exists( $sType, ExtDynamicPageList::$options )
				) {
					continue;
				}
			}
			...
			switch ( $sType ) {
				...
				case 'namespace':
					...
					foreach ( $aParams as $sParam ) {
						...
						} elseif ( in_array( $sNs, array_keys( ExtDynamicPageList::$options['namespace'] ) ) ) {
							$aNamespaces[] = $sNs;
							$bSelectionCriteriaFound = true;
						} else {
			...
			switch ( $sType ) {
				...
				case 'scroll':
					if ( in_array( $sArg, ExtDynamicPageList::$options['scroll'] ) ) {
						...
						// if scrolling is active we adjust the values for ceratin other parameters
			...
			switch ( $sType ) {
				...
				case 'linksto':
					$problems = self::getPageNameList(
						'linksto', $sArg, $aLinksTo,
						$bSelectionCriteriaFound, $logger, true
					);
					if ( $problems != '' ) {
						return $problems;
					}
				...
				case 'notlinksfrom':
					$problems = self::getPageNameList(
						'notlinksfrom', $sArg, $aNotLinksFrom,
						$bSelectionCriteriaFound, $logger, true
					);
					if ( $problems != '' ) {
						return $problems;
				...
				case 'linkstoexternal':
					$problems = self::getPageNameList(
						'linkstoexternal', $sArg, $aLinksToExternal,
						$bSelectionCriteriaFound, $logger, false
					);
					if ( $problems != '' ) {
						return $problems;
					}
		...
		*/
		...
		/**
		 * If we include the Uncategorized, we need the 'dpl_clview':
		 * VIEW of the categorylinks table where we have cl_to='' (empty string)
		 * for all uncategorized pages.
		 * This VIEW must have been created by the administrator of the
		 * MediaWiki DB at installation. See the documentation.
		 */
		$sDplClView = '';
		if ( $bIncludeUncat ) {
		...
		if ( $sPageListMode == 'category' && ( $bAddCategories || $bAddEditDate
		...
		$sSqlPage_touched = '';
		$sSqlCalcFoundRows = '';
		if ( !ExtDynamicPageList::$allowUnlimitedResults &&
			$sGoal != 'categories' &&
		...
		foreach ( $aOrderMethods as $sOrderMethod ) {
			switch ( $sOrderMethod ) {
				...
				case 'sortkey':
					...
					$sSqlNsIdToText .= ' END';
					// If cl_sortkey is null (uncategorized page), generate a
					// sortkey in the usual way (full page name, underscores replaced with spaces).
					...
					if ( count( $aIncludeCategories ) + count( $aExcludeCategories ) > 0 ) {
						if (
							in_array( 'category', $aOrderMethods ) &&
							( count( $aIncludeCategories ) + count( $aExcludeCategories ) > 0 )
						) {
		...
		if ( count( $aLinksTo ) > 0 ) {
			...
			foreach ( $aLinksTo as $linkGroup ) {
				...
				foreach ( $linkGroup as $link ) {
					...
					if ( $bIgnoreCase ) {
						$sSqlCond_page_pl .= ' AND LOWER(pl.pl_title)' . $operator . 'LOWER(' . $dbr->addQuotes( $link->getDBkey() ) . '))';
					} else {
		...
		if ( count( $aLinksTo ) > 1 ) {
			...
			foreach ( $aLinksTo as $linkGroup ) {
				...
				foreach ( $linkGroup as $link ) {
					...
					if ( $bIgnoreCase ) {
						$sSqlCond_page_pl .= ' AND LOWER(pagelinks.pl_title)' . $operator . 'LOWER(' . $dbr->addQuotes( $link->getDBkey() ) . ')';
					} else {
		...
		if ( count( $aNotLinksTo ) > 0 ) {
			...
			foreach ( $aNotLinksTo as $links ) {
				foreach ( $links as $link ) {
					...
					if ( $bIgnoreCase ) {
						$sSqlCond_page_pl .= ' AND LOWER(' . $sPageLinksTable . '.pl_title)' . $operator . 'LOWER(' . $dbr->addQuotes( $link->getDBkey() ) . '))';
					} else {
		...
		if ( $sTitleIs != '' ) {
			if ( $bIgnoreCase ) {
				$sSqlWhere .= ' AND LOWER(' . $sPageTable . '.page_title) = LOWER(' . $dbr->addQuotes( $sTitleIs ) . ')';
			} else {
		...
		if ( count( $aTitleMatch ) > 0 ) {
			...
			foreach ( $aTitleMatch as $link ) {
				...
				if ( $acceptOpenReferences ) {
					if ( $bIgnoreCase ) {
						$sSqlWhere .= 'LOWER(pl_title)' . $sTitleMatchMode . strtolower( $dbr->addQuotes( $link ) );
					} else {
						$sSqlWhere .= 'pl_title' . $sTitleMatchMode . $dbr->addQuotes( $link );
					}
				} else {
					if ( $bIgnoreCase ) {
						$sSqlWhere .= 'LOWER(' . $sPageTable . '.page_title)' . $sTitleMatchMode . strtolower( $dbr->addQuotes( $link ) );
					} else {
		...
		if ( count( $aNotTitleMatch ) > 0 ) {
			...
			foreach ( $aNotTitleMatch as $link ) {
				...
				if ( $acceptOpenReferences ) {
					if ( $bIgnoreCase ) {
						$sSqlWhere .= 'LOWER(pl_title)' . $sNotTitleMatchMode . 'LOWER(' . $dbr->addQuotes( $link ) . ')';
					} else {
						$sSqlWhere .= 'pl_title' . $sNotTitleMatchMode . $dbr->addQuotes( $link );
					}
				} else {
					if ( $bIgnoreCase ) {
						$sSqlWhere .= 'LOWER(' . $sPageTable . '.page_title)' . $sNotTitleMatchMode . 'LOWER(' . $dbr->addQuotes( $link ) . ')';
					} else {
		...
		if ( $sSqlCalcFoundRows != '' ) {
			$res = $dbr->query( 'SELECT FOUND_ROWS() AS rowcount', __METHOD__ );
		...
		// ###### SHOW OUTPUT ######
		$listMode = new DPLListMode(
			$sPageListMode, $aSecSeparators, $aMultiSecSeparators, $sInlTxt,
			$sListHtmlAttr, $sItemHtmlAttr, $aListSeparators, $iOffset,
			$iDominantSection
		);

		$hListMode = new DPLListMode(
			$sHListMode, $aSecSeparators, $aMultiSecSeparators, '',
			$sHListHtmlAttr, $sHItemHtmlAttr, $aListSeparators, $iOffset,
			$iDominantSection
		);

		$dpl = new DPL(
			$aHeadings, $bHeadingCount, $iColumns, $iRows, $iRowSize,
			$sRowColFormat, $aArticles, $aOrderMethods[0], $hListMode,
			$listMode, $bEscapeLinks, $bAddExternalLink, $bIncPage,
		...
		self::defineScrollVariables(
			$firstNamespaceFound, $firstTitleFound, $lastNamespaceFound,
			$lastTitleFound, $scrollDir, $iCount, "$dplElapsedTime ($nowTimeStamp)",
			$rowcount, $dpl->getRowCount()
		);
...
*/
		...
		if ( $bReset[4] ) { // LINKS
			// we trigger the MediaWiki parser to find links, images, categories
			// etc. which are contained in the DPL output
	...
	private static function getPageNameList( $cmd, $text, &$aLinks, &$bSelectionCriteriaFound, $logger, $mustExist = true ) {
		...
		foreach ( $pages as $page ) {
			...
			// sequences like %1a would be translated to hex chars; we avoid this by escaping the char after the %
	...
	private static function validParametersList() {
		...
		foreach ( ExtDynamicPageList::$validParametersForRichnessLevel as $level => $p ) {
			$plist .= $p;
			if ( $level >= ExtDynamicPageList::$functionalRichness ) {
				break;
			}
		}
		return $plist;
	...
	private static function durationTime( $t ) {
		...
		if ( $t < 3600 ) {
			return '00:' . str_pad( floor( $t / 60 ), 2, '0', STR_PAD_LEFT ) .
					':' . str_pad( floor( fmod( $t, 60 ) ), 2, '0', STR_PAD_LEFT );
		}
		if ( $t < 86400 ) {
			return str_pad( floor( $t / 3600 ), 2, '0', STR_PAD_LEFT ) . ':' .
					str_pad( floor( fmod( floor( $t / 60 ), 60 ) ), 2, '0', STR_PAD_LEFT ) .
					':' . str_pad( fmod( $t, 60 ), 2, '0', STR_PAD_LEFT );
		}
		if ( $t < 2 * 86400 ) {
			return '1 day';
		}
