...
class ExternalDomainPatternParserTest extends MediaWikiIntegrationTestCase {
	 ...
	 */
	public function testParseDomainPattern( string $domain, string $expectedDomainIndex, string $expectedPath ): void {
	 ...
	 */
	public function testUnsupportedDomainPatterns( string $domain, string $expectedDomainIndex, string $expectedPath ): void {
		$result = LinkFilter::makeIndexes( $domain )[0];
		$actualDomain = str_replace( '%25', '%', $result[0] );
		$actualPath = str_replace( '%25', '%', $result[1] );
